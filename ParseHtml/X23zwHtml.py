...
class X23zwHtml:
    ...
    def detail(self,url, name, author, types, typesName, updateChapter):
        ...
        img_src = img_tag['src']
        OSSUtil.upload("https://www.x23zw.com/"+img_src)
        intro = soup.find(id='intro')
        info = intro.text

        # 获取作者
        author = soup.find("div", id="maininfo").find("div", id="info").find("p").a.text

        # 获取最新章节
        latest_chapter = soup.find("div", id="maininfo").find("div", id="info").find_all("p")[3].a.text
        Description = intro.text
        world = self.worldService.select(name);
        if (world is None):
            world = self.worldService.select(name + "(" + author + ")");
            if (world is None):
                world = self.worldService.add(name, info, Description, "\\" + "sotry" + img_src, types, typesName,
                                              author)
        else:
        ...
        if (story is None):
            story = self.storyService.add(world.id, world.name, author, name, info, Description,
                                          "\\"+ "sotry" + img_src, types, typesName, 1, latest_chapter)
    ...
    def addChapter(self,url, pChapter, story, chapterName,upId):
        if (self.acaheFileUtil.exists(url)):
            ...
            responText = HtmlUtil.get_api_data("https://www.x23zw.com"+url)
            if (responText == None):
                upId=0;
                LogUtil.error(f"url:{url} sotry:{story.name} chapterName:{chapterName} 获取数据失败")
                return;
            else:
                OSSUtil.uploadChapter(url)
            file_path = self.acaheFileUtil.save(responText, url);
        soup = BeautifulSoup(responText, 'html.parser')
